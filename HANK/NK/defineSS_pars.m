...
mpar.in         = 0.00063; % Prob. to become top 1 percent
mpar.out        = 0.0625;  % Prob. to fall out of 1 percent
...
par.alphay       = 1-(par.Labshr)/par.mu;  % capital elasticity
...
par.thetay     = 1.00;     % scale Y
 ...
 %scaley=1/par.mu*(2/3)/(1+lyle_ratio); % thetay*(1-alphay)
 
% scalen=1/(1-par.mu)*(2/3)/(1+1/lyle_ratio); % thetan*(1-alphan)
...
par.phiw=par.thetaw*(par.epsilon_w-1)*par.tau/((1-par.thetaw)*(1-par.beta*par.thetaw)); % wage adjustment cost
