...
hhcind=[(Ny+16):(Ny+15+length(targets.cinds))]';
...
%Yss=[invmutil(mutil_c(:)); invmutil(Vk(:)); log(par.Q); log(par.PI); log(Output);...
 ...
 %   log(targets.B) ;log(targets.Inv); log(par.R); log(targets.C); log(grid.ly); log(grid.le); log(grid.Mg);
...
%giniWminus=Controlminus(giniWind);
%Top10minus=Controlminus(Top10ind);
%Top1pctminus=Controlminus(Top1pctind);
...
piy=mc*Yminus-W*lyminus-(Rminus+par.delta)*Kminus; % dividends of production
pin=Yminus*(1-mc)-W*leminus;
...
taxrevenue =(1-tau).*W.*Nminus;% +(1-par.tau).*Profitminus;
...
% % wealth gini
% 
% LHS(nx+giniWind)=giniWminus;
% 
% % Net worth Gini
% mplusk=meshes.k(:)+meshes.m(:);
% [mplusk, IX]       = sort(mplusk);
% moneycapital_pdf   = JD_new(IX);
% 
% S                  = cumsum(moneycapital_pdf.*mplusk)';
% S                  = [0 S];
% RHS(nx+giniWind)      = 1-(sum(moneycapital_pdf.*(S(1:end-1)+S(2:end))')/S(end));
% 
% % Top 10 wealth share
% 
% LHS(nx+Top10ind)=Top10minus;
% 
% RHS(nx+Top10ind)=sum((cumsum(moneycapital_pdf)>0.9).*mplusk.*moneycapital_pdf)/sum(moneycapital_pdf.*mplusk);
% 
% % Top 1 pct income share
% 
% %LHS(nx+Top1pctind)=Top1pctminus;
% 
% %totinc=inc.labor(:)+inc.money(:)+inc.rent(:)-meshes.m(:);
% 
% [totinc, IX]       = sort(totinc);
% totinc_pdf   = JDminus(IX);
% 
% RHS(nx+Top1pctind)=sum((cumsum(totinc_pdf)>=0.99).*totinc.*totinc_pdf)/sum(totinc_pdf.*totinc);
% 
