function [IRF_Cdecomp,IRF_C]=partial_decomp_NK(SS,T,Tf,Irb,Ira,Ilab,Itau)
...
%SS='../HANK/steadystates/NKfund_60_15.mat';
%T=101;
%Tf=24;
...
load(SS,'IRF_C','P_H','gx','hx','css','Vk','mutil_c','par','grid','meshes','mpar','joint_distr','targets','NN')
...
RB=par.RB*exp(Irb*IRF_decomp(mpar.numstates-1,1:end-1));
...
for tb=1:T-1
  ...
  NW=Htot*W(t); % little adjustment because in egm update procedure household picks x=c-G(h,n)
...
for t=1:Tf-1
  ...
  waitbar(t/(Tf-1),f,sprintf('%i ',round(100*t/(Tf-1),0)))
